...
void z8cwiz3()
{
    ...
    set<ll> pref_st;
    ll p = 0;
    vector<ll> pref_mex(n);
    fpve(0, n - 1, 1)
    {
        pref_st.ins(v[i]);
        while (pref_st.count(p))
            p++;
        pref_mex[i] = p;
    }
    set<ll> suff_st;
    ll q = 0;
    vector<ll> suff_mex(n);
    fnve(n - 1, 0, 1)
    {
        suff_st.ins(v[i]);
        while (suff_st.count(q))
            q++;
        suff_mex[i] = q;
    }
    vector<pair<ll, ll>> res_mex_idx;
    fops(i, 0, n - 2)
    {
        if (pref_mex[i] == suff_mex[i + 1])
        {
            res_mex_idx.pb({1, i + 1});
            res_mex_idx.pb({i + 2, n});
            brk;
        }
    }
    if (!res_mex_idx.sz)
    ...
    CE(2);
    every(res_x, res_mex_idx)
        CE(res_x.fs << " " << res_x.sc);
}
