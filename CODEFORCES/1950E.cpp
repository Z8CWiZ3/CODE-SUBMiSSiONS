...
void z8cwiz3()
{
    ...
    if (is_prime(n))
    {
        ...
        tmp1 += s.ft;
        tmp2 += s.bk;
        if (chk_str(s, tmp1) || chk_str(s, tmp2))
            CE(1);
        else
            CE(n);
        return;
    }
    set<ll> st_div = every_divs(n);
    every(res_len, st_div)
    {
        str s1 = s.substr(0, res_len);
        str s2 = s.substr(n - res_len, n);
        if (chk_str(s, s1) || chk_str(s, s2))
        {
            CE(res_len);
            return;
        }
