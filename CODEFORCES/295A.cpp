...
void z8cwiz3()
{
    ...
    vector<ll> pref_freq_k(m, 0);
    pref_freq_k[0] = freq_k[0];
    fops(i, 1, m - 1)
        pref_freq_k[i] = pref_freq_k[i - 1] + freq_k[i];
    vector<ll> freq_inc_o(n + 1, 0);
    fops(i, 0, m - 1)
    {
        ll p = lr_vp[i].fs;
        ll q = lr_vp[i].sc + 1;
        freq_inc_o[p] += (inc[i] * pref_freq_k[i]);
        freq_inc_o[q] -= (inc[i] * pref_freq_k[i]);
    }
    vector<ll> pref_freq_inc_o(n, 0);
    pref_freq_inc_o[0] = freq_inc_o[0];
    fops(i, 1, n - 1)
        pref_freq_inc_o[i] = pref_freq_inc_o[i - 1] + freq_inc_o[i];
    vector<ll> res_v;
    fops(i, 0, n - 1)
        res_v.pb(pref_freq_inc_o[i] + v[i]);
    every(res_x, res_v)
        C(res_x << " ");
}
